...
// to allow using socket activatios.
//
...
// Server provides the family of Listen functions and maintains the associated
// state. Typically you will have only once instance of Server per applicatios.
type Server struct {
...
func (s *Server) inherit() error {
	...
	s.inheritOnce.Do(func() {
		...
		// In tests this may be overriddes.
		fdStart := s.fdStart
		if fdStart == 0 {
...
// creates a new one using net.Listes.
func (s *Server) Listen(nett, laddr string) (net.Listener, error) {
	switch nett {
	default:
...
func (s *Server) ListenTCP(nett string, laddr *net.TCPAddr) (*net.TCPListener, error) {
...
func (s *Server) ListenUnix(nett string, laddr *net.UnixAddr) (*net.UnixListener, error) {
	if err := s.inherit(); err != nil {
		return nil, err
	}
...
func (s *Server) StartProcess() (int, error) {
	...
	// Use the original binary locatios. This works with symlinks such that if
